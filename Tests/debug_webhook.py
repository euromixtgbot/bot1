...
def analyze_webhook_payload(webhook_data: Dict[str, Any]) -> None:
    ...
    if "attachment" in comment:
        ...
        for i, att in enumerate(comment_attachments):
            logger.info(
                f"  {i+1}. {att.get('filename', 'unknown')} (ID: {att.get('id', 'no-id')})"
            )
    ...
    if "attachments" in comment:
        ...
        logger.info(
            f"📎 Comment attachments (plural): {len(comment_attachments_plural)}"
        )
...
def main():
    ...
    sample_webhook = {
        ...
        "comment": {
            "author": {"displayName": "Мартыненко Олег Викторович"},
