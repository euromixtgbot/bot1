...
async def test_file_download_simulation():
    ...
    try:
        # Імітуємо файл Excel
        excel_content = (
            b"PK\\x03\\x04\\x14\\x00\\x00\\x00\\x08\\x00"  # Початок Excel файлу
        )
        filename = "test.xlsx"
        ...
        if "issue" in mock_webhook and "fields" in mock_webhook["issue"]:
            if "attachment" in mock_webhook["issue"]["fields"]:
                ...
                if issue_attachments:
                    ...
                    print(
                        f"✅ Знайдено {len(issue_attachments)} вкладень на рівні issue"
                    )
...
async def run_comprehensive_diagnosis():
    ...
    if all_passed:
        print(
            "✅ Всі тести пройшли! Проблема може бути в реальних webhooks або налаштуваннях."
        )
    else:
