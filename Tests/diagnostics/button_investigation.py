...
def test_file_retention_system():
    ...
    try:
        ...
        save_result = save_registration_state(
            test_user_id, registration_data, "confirm"
        )
        ...
        print(
            f"Завершення реєстрації: {'✅ Успішно' if complete_result else '❌ Помилка'}"
        )
        ...
        print(
            f"Завантаження після завершення: {'❌ Не завантажується (правильно)' if not loaded_after_complete else '⚠️ Все ще завантажується'}"
        )
