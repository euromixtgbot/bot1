...
def find_user_by_phone(phone: str) -> Optional[Tuple[Dict[str, Union[int, float, str]], int]]:
    """
...
@handle_google_api_errors
def find_user_by_telegram_id(telegram_id: str) -> Optional[Tuple[Dict[str, Union[int, float, str]], int]]:
    """
...
def update_user_telegram(row: int, telegram_id: str, telegram_username: str) -> None:
    ...
    if "telegram_id" in _COLUMN_INDEX:
        col_letter = chr(64 + _COLUMN_INDEX["telegram_id"])  # Convert column index to letter
        batch_updates.append({
            'range': f"{col_letter}{row}",
            'values': [[normalized_telegram_id]]
        })
    if "telegram_username" in _COLUMN_INDEX and telegram_username:
        col_letter = chr(64 + _COLUMN_INDEX["telegram_username"])  # Convert column index to letter
        batch_updates.append({
            'range': f"{col_letter}{row}",
...
def add_new_user(record: Dict[str, str]) -> int:
    ...
    if not any(field in record and record[field] for field in required_any_one):
        raise ValueError("At least one of required fields must be present: full_name, telegram_id or mobile_number")
