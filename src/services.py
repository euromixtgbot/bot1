...
async def _make_request(method: str, url: str, **kwargs) -> Dict[str, Any]:
    ...
    for attempt in range(max_retries):
        try:
            ...
            try:
                ...
                logger.error(
                    f"Jira API error response: {json.dumps(error_details, indent=2, ensure_ascii=False)}"
                )
                ...
                if e.response.status_code == 400 and isinstance(error_details, dict):
                    ...
                    for field_name, error_text in errors_dict.items():
                        logger.error(
                            f"Field validation error for '{field_name}': {error_text}"
                        )
                        ...
                        if field_name in [
                            ...
                            logger.error(
                                f"❌ Помилка валідації для кастомного поля {field_label} ({field_name}): {error_text}"
                            )
    ...
    raise JiraApiError(
        f"Request failed after {max_retries} attempts with no specific error"
    )
...
def build_jira_payload(bot_vars: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
    ...
    for var_name, value in bot_vars.items():
        ...
        if jira_field == "summary":
            ...
            logger.info("Converting description to Atlassian Document Format")
        ...
        # Обробка кастомних полів
            ...
            if isinstance(value, dict) and ("name" in value or "id" in value):
                ...
                if field_type:
                    ...
                    logger.info(
                        f"Using field mapping for {jira_field}: {str_value} -> {fields[jira_field]}"
                    )
                else:
                    fields[jira_field] = {"name": str_value}
                    logger.warning(
                        f"Unknown field type for {jira_field}, using default format: {fields[jira_field]}"
                    )
        else:
            fields[jira_field] = value
    ...
    if (
        "issuetype" in fields
        and isinstance(fields["issuetype"], dict)
        and "name" not in fields["issuetype"]
    ):
...
async def create_jira_issue(fields: Dict) -> str:
    ...
    missing_fields = [
        field for field in required_fields if field not in fields.get("fields", {})
    ]
    ...
    if project_field and not isinstance(project_field, dict):
        ...
        logger.warning(
            f"Автоматически исправлен формат поля project: {project_field} -> {fields['fields']['project']}"
        )
    ...
    if issuetype_field and not isinstance(issuetype_field, dict):
        ...
        logger.warning(
            f"Автоматически исправлен формат поля issuetype: {issuetype_field} -> {fields['fields']['issuetype']}"
        )
    ...
    while attempt <= max_attempts:
        try:
            ...
            if attempt == 1:
                logger.info(
                    f"Отправка запроса на создание задачи в Jira (попытка {attempt}): {url}"
                )
            else:
                logger.info(
                    f"Повторная попытка ({attempt}) создания задачи с модифицированными полями: {url}"
                )
            ...
            if reporter_field:
                ...
                if JIRA_REPORTER_ACCOUNT_ID:
                    ...
                    fields["fields"]["reporter"] = {
                        "accountId": JIRA_REPORTER_ACCOUNT_ID
                    }
                    logger.info(
                        f"Using default reporter accountId: {JIRA_REPORTER_ACCOUNT_ID}"
                    )
                    ...
                    logger.info(
                        f"Converting reporter format: {reporter_field} -> {fields['fields']['reporter']}"
                    )
                    ...
                    if isinstance(reporter_field["accountId"], dict):
                        ...
                        logger.info(
                            f"Fixed nested reporter accountId format: {accountId}"
                        )

            # Process custom fields
            for cf_id, cf_name in custom_fields.items():
                if (
                    cf_id in fields.get("fields", {})
                    and cf_id not in problematic_fields
                ):
                    ...
                    if isinstance(value, dict) and ("id" in value or "name" in value):
                        logger.info(
                            f"Field {cf_name} ({cf_id}) already in correct format: {value}"
                        )
                    else:
                        ...
                        fields["fields"][cf_id] = get_field_value_by_name(
                            cf_id, str_value
                        )
                        logger.info(
                            f"Using field mapping for {cf_name} ({cf_id}): {str_value} -> {fields['fields'][cf_id]}"
                        )

            # Ensure description is in Atlassian Document Format
            if "description" in fields.get("fields", {}) and not isinstance(
                fields["fields"]["description"], dict
            ):
            ...
            response = await _make_request(
                "POST", url, json=fields, headers=HEADERS_JSON
            )
            ...
            try:
                ...
                verification = await _make_request(
                    "GET", verify_url, headers=HEADERS_JSON
                )
                if verification and verification.get("key") == issue_key:
                    logger.info(
                        f"Успешно подтверждено создание задачи с ключом: {issue_key}"
                    )
                else:
                    logger.warning(
                        f"Created issue {issue_key} but couldn't verify it properly"
                    )
            except Exception as e:
                # Log but don't fail if verification has issues
                logger.warning(
                    f"Created issue {issue_key} but verification failed: {str(e)}"
                )

            # Log which fields were removed in successful attempt
            if problematic_fields:
                logger.info(
                    f"✅ Задача успішно створена після видалення проблемних полів: {problematic_fields}"
                )
            else:
                logger.info("✅ Задача успішно створена з усіма полями")
            ...
            for field_id, keywords in validation_errors.items():
                ...
                if field_id in problematic_fields or field_id not in fields.get(
                    "fields", {}
                ):
                    continue
                ...
                if field_id == "reporter" and '"reporter":' in error_message_lower:
                    ...
                    logger.info(
                        "Обнаружена ошибка валидации для поля reporter по точному совпадению в JSON"
                    )
                    break
                ...
                if any(keyword.lower() in error_message_lower for keyword in keywords):
                    ...
                    logger.info(
                        f"Обнаружена ошибка валидации для поля {field_name} ({field_id})"
                    )
                    break
            ...
            if field_to_remove and attempt < max_attempts:
                ...
                if field_to_remove == "reporter":
                    if JIRA_REPORTER_ACCOUNT_ID:
                        logger.info(
                            f"Заменяю проблемное поле reporter на значение по умолчанию: {JIRA_REPORTER_ACCOUNT_ID}"
                        )
                        fields["fields"]["reporter"] = {
                            "accountId": JIRA_REPORTER_ACCOUNT_ID
                        }
                        ...
                        logger.info(
                            "Удаляю проблемное поле reporter, так как JIRA_REPORTER_ACCOUNT_ID не установлен"
                        )
                        if "reporter" in fields.get("fields", {}):
                            del fields["fields"]["reporter"]
                else:
                    logger.info(
                        f"Удаляю проблемное поле {field_to_remove} и повторяю попытку"
                    )
                ...
                logger.error(
                    f"Оригинальный payload: {json.dumps(original_fields, indent=2, ensure_ascii=False)[:200]}..."
                )
                raise JiraApiError(
                    f"Failed to create Jira issue after {attempt} attempts: {error_message}"
                )

        except Exception as e:
            error_msg = f"Failed to create Jira issue: {str(e)}"
            logger.error(
                f"{error_msg}. Payload: {json.dumps(fields, indent=2, ensure_ascii=False)[:200]}..."
            )
            raise JiraApiError(error_msg) from e

    # This point should never be reached, but added for type safety
    raise JiraApiError(
        f"Failed to create Jira issue after {max_attempts} attempts with no specific error"
    )
...
async def find_open_issues(telegram_id: str) -> List[Dict[str, str]]:
    ...
    logger = logging.getLogger(__name__)
    logger.info(
        f"find_open_issues: пошук задач для Telegram ID: '{telegram_id}' (нормалізовано: '{normalized_id}')"
    )
    ...
    response = await _make_request(
        "GET", url, params={"jql": jql, "fields": "status"}, headers=HEADERS_JSON
    )

    # Додаємо логування відповіді
    logger.info(f"find_open_issues: відповідь від Jira: {str(response)[:200]}...")
    logger.info(
        f"find_open_issues: кількість issues в відповіді: {len(response.get('issues', []))}"
    )

    # Логуємо повну відповідь від Jira для діагностики
    logger.info(
        f"find_open_issues: відповідь від Jira: {json.dumps(response)[:200]}..."
    )
    ...
    if result:
        for issue in result:
            logger.info(
                f"find_open_issues: задача {issue['key']} зі статусом {issue['status']}"
            )
...
async def find_done_issues(telegram_id: str) -> List[Dict[str, str]]:
    ...
    logger = logging.getLogger(__name__)
    logger.info(
        f"find_done_issues: пошук завершених задач для Telegram ID: '{telegram_id}' (нормалізовано: '{normalized_id}')"
    )
    ...
    # Логування відповіді
    logger.info(
        f"find_done_issues: кількість завершених задач в відповіді: {len(response.get('issues', []))}"
    )
    ...
    if result:
        for issue in result:
            logger.info(
                f"find_done_issues: задача {issue['key']} зі статусом {issue['status']}"
            )
...
async def add_comment_to_jira(
    issue_key: str, comment: str, author_name: Optional[str] = None
) -> None:
    """
    ...
    payload = {
        "body": {
            ...
            "content": [
                {
                    "type": "paragraph",
                    "content": [{"text": full_comment, "type": "text"}],
                }
            ],
        }
    }
...
async def get_issue_status(issue_key: str) -> Dict[str, str]:
    ...
    return {"name": status["name"], "category": status["statusCategory"]["name"]}
...
async def get_full_issue_info(issue_key: str) -> Dict[str, Any]:
    ...
    if created_date:
        ...
        try:
            ...
            created_date = dt.strftime(
                "%d.%m.%Y, %H:%М"
            )  # Fixed Cyrillic 'М' to Latin 'M'
        except Exception:
            pass
    ...
    return {
        ...
        "department": fields_data.get(
            "customfield_10065", "Невідомо"
        ),  # Додаємо поле департаменту
...
def _ensure_correct_custom_fields_format(payload: Dict[str, Any]) -> None:
    ...
    field_mappings = {
        "customfield_10069": JIRA_FIELD_MAPPINGS.get(
            "customfield_10069", {}
        ),  # DIVISION_ID_MAPPINGS
        "customfield_10065": JIRA_FIELD_MAPPINGS.get(
            "customfield_10065", {}
        ),  # DEPARTMENT_ID_MAPPINGS
        "customfield_10068": JIRA_FIELD_MAPPINGS.get(
            "customfield_10068", {}
        ),  # SERVICE_ID_MAPPINGS
    }
            ...
            if isinstance(field_value, dict):
                ...
                if "id" in field_value:
                    if len(field_value) > 1 or field_value.get("name") == field_id:
                        ...
                        logger.info(
                            f"Field {field_name} ({field_id}) already in correct format with ID: {fields[field_id]}"
                        )
                    ...
                    logger.info(
                        f"Field {field_name} ({field_id}) corrected to use name: {fields[field_id]}"
                    )
                    ...
                    for key, value in mapping.items():
                        if key == name_value or value.get("name") == name_value:
                            ...
                            if value.get("id"):
                                ...
                                logger.info(
                                    f"Заменено name на id для {field_name}: {name_value} -> {fields[field_id]}"
                                )
                            break

                    if not found:
                        logger.warning(
                            f"⚠️ Имя '{name_value}' для поля {field_name} не найдено в маппинге!"
                        )
                else:
                    logger.warning(
                        f"Field {field_name} ({field_id}) has invalid format: {field_value}"
                    )
            else:
                ...
                if str_value.isdigit():
                    ...
                    logger.info(
                        f"Field {field_name} ({field_id}) converted to ID format: {fields[field_id]}"
                    )
                elif str_value in mapping:
                    # Используем ID из маппинга
                    fields[field_id] = {"id": mapping[str_value].get("id", "")}
                    logger.info(
                        f"Field {field_name} ({field_id}) mapped from name to ID: {str_value} -> {fields[field_id]}"
                    )
                else:
                    # Если не нашли в маппинге, используем как имя
                    fields[field_id] = {"name": str_value}
                    logger.info(
                        f"Field {field_name} ({field_id}) converted to name format: {fields[field_id]}"
                    )
                    logger.warning(
                        f"⚠️ Имя '{str_value}' для поля {field_name} не найдено в маппинге!"
                    )
...
async def find_user_by_jira_issue_key(issue_key: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        response = await get_issue_details(
            issue_key, ["customfield_10145"]
        )  # telegram_id field
        ...
        if not telegram_id:
            logger.info(
                f"No Telegram ID found for issue {issue_key} - likely created manually in Jira web interface"
            )
            return None
...
async def add_internal_note_to_jira(issue_key: str, note_text: str) -> None:
    ...
    payload = {
        "body": {
            ...
            "version": 1,
            "content": [
                {"type": "paragraph", "content": [{"text": note_text, "type": "text"}]}
            ],
        },
    ...
    try:
        ...
        logger.error(
            f"Помилка при додаванні внутрішньої примітки до {issue_key}: {str(e)}"
        )
        raise JiraApiError(f"Failed to add internal note: {str(e)}")


async def add_comment_with_file_reference_to_jira(
    issue_key: str,
    comment: str,
    ...
    if filename and file_content:
        try:
            attachment_response = await attach_file_to_jira(
                issue_key, filename, file_content
            )
            ...
            if attachment_response and len(attachment_response) > 0:
                ...
                if attachment_id:
                    attachment_url = (
                        f"{JIRA_BASE_URL}/secure/attachment/{attachment_id}/{filename}"
                    )
                    logger.info(f"URL файлу: {attachment_url}")
        except Exception as e:
            logger.error(
                f"Помилка при прикріпленні файлу {filename} до {issue_key}: {str(e)}"
            )
    ...
    if attachment_url and filename:
        ...
        adf_content = [
            {
                "type": "paragraph",
                "content": [{"text": full_comment_text, "type": "text"}],
            }
        ]
        ...
        adf_content.append(
            {
                "type": "paragraph",
                "content": [
                    {"type": "text", "text": "📎 Прикріплено файл: "},
                    {
                        "type": "text",
                        "text": filename,
                        "marks": [
                            {
                                "type": "link",
                                "attrs": {"href": attachment_url, "title": filename},
                            }
                        ],
                    },
                ],
        ...
        payload = {"body": {"type": "doc", "version": 1, "content": adf_content}}
        ...
        try:
            ...
            logger.error(
                f"Помилка при додаванні коментаря з посиланням на файл до {issue_key}: {str(e)}"
            )
            raise
    else:
        # Якщо немає файлу або URL, створюємо звичайний коментар
        full_comment = (
            f"{header_text}{comment}" if comment else f"{header_text}Файл прикріплено"
        )
        await add_comment_to_jira(
            issue_key, full_comment, None
        )  # author_name вже додано до тексту
